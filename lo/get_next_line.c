...
int	main(int ac, char *av[])
{
	...
	lst = counter(fd);
	vars.mlx = mlx_init();
	vars.win = mlx_new_window(vars.mlx, 1000, 1000, "pp");
	mlx_hook(vars.win, 2, 1L<<0, ft_close, &vars);
	mlx_key_hook(vars.win, ft_close, &vars);
	mlx_key_hook(vars.win, ft_move, &vars);
	img = mlx_xpm_file_to_image(vars.mlx, path, &img_height, &img_width);
	mlx_put_image_to_window(vars.mlx, vars.win, img, 100, 10);
	mlx_loop(vars.mlx);
}
